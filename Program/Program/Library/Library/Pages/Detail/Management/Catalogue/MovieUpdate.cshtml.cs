...
namespace Library.Pages.Detail.Management.Catalogue
{
    public class MovieUpdateModel : PageModel
    {
        ...
        [BindProperty]
        [Required]
        [RegularExpression(@"[0-9]{1,}", ErrorMessage = "There went something wrong please reclick the item that you wanted to update")]
        ...
        public IActionResult OnPost()
        {
            ...
            {
                ItemManagement management = new ItemManagement(new DBConnection());
                if (management.UpdateItem(itemID, Name, ISBN, cost, Language, Description, SubtitleLanguage, Producer, timeInMin,Demographic))
                {
                    return RedirectToPage("/Detail/Management/Catalogue/Catalogue");
                }
                else
